...
**Francesco 14.11.2025**: Guaranteed Smart TV rendering for the monthly goal progress bar. Added coarse-pointer detection that swaps the animated Framer Motion fill with a simplified static gold gradient, raised the minimum visible width on TVs, and removed global TV-specific animation overrides so every device displays a visible bar even if performance costs increase.
