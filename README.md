...
- **Endpoint**: `http://185.92.192.127:3002/api/leaderboard`
- **Method**: GET
...
- `NEXT_PUBLIC_LEADERBOARD_API_URL` - Override the external leaderboard API URL (default: `http://185.92.192.127:3002/api/leaderboard`)
...
**Francesco 14.11.2025**: Tweaked Smart TV experience for the monthly goal progress bar. Added dedicated `tv-progress-*` utility classes in the progress component so coarse-pointer devices can fall back to a static gold fill while laptops retain full Framer Motion gradients, glow, and shine effects. Updated `globals.css` to stop clamping every animation to 0.2s on TVs, disable the heavy glow/shine layers, and keep quick transitions without sacrificing visibility.
